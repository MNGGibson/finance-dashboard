    ...
    with conn, conn.cursor() as cur:
        if args.find:
            cur.execute(
                ...
                "WHERE description ILIKE %s ORDER BY posted DESC LIMIT 25", (f"%{args.find}%",))
        ...
        if args.auto:
            cur.execute("UPDATE transactions SET category_manual = false, updated_at = now() WHERE id = %s",
                        (args.transaction_id,))
        else:
            cur.execute("UPDATE transactions SET category = %s, category_manual = true, updated_at = now() "
                        "WHERE id = %s", (args.category, args.transaction_id))
        if cur.rowcount == 0:
