...
- **Forecasting** — leads with a **cash forecast**: today's cash plus every detected paycheck and bill laid out day by day for the next 30, 60 or 90 days. It reports the lowest balance ahead and its date, and how much is free to send to debt while staying above a cash floor you choose. A "send to debt today" what-if and the planning levers both redraw it. The engine is in `forecast.py` (pure pandas, no Streamlit). Below that: sidebar assumptions pre-filled from your synced data (editable, including pay schedule and fixed bills), a debt-focus selector, and three levers (cut spending / more side income / raise) that live-recompute a payoff projection against the account's real balance and any promotional-APR deadline.
